...
<div class="py-12 bg-slate-50 dark:bg-slate-950 min-h-[60vh] transition-colors duration-300" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div class="flex flex-wrap items-center justify-center gap-3 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <a href="{{ url('/galeri') }}" 
               class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 {{ !request()->filled('category') ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-600/10' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                Semua
            </a>
            @foreach($categories as $category)
                <a href="{{ url('/galeri?category=' . urlencode($category)) }}" 
                   class="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 {{ request()->query('category') === $category ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-600/10' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                    {{ $category }}
                </a>
            @endforeach
        </div>
