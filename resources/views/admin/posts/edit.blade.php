<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.posts.index') }}" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-2xl border border-gray-150 dark:border-gray-700 p-6 sm:p-10">
                ...
                <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <!-- Title -->
                    <div class="space-y-1">
                        <label for="title" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Judul Kegiatan</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Category -->
                        <div class="space-y-1">
                            <label for="category" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Kategori</label>
                    ...
                    <!-- Content -->
                    <div class="space-y-1">
                        <label for="content" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Deskripsi / Konten Kegiatan</label>
                        <textarea name="content" id="content" rows="8" 
                                  class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 @error('content') border-red-500 @enderror" 
                                  placeholder="Tulis deskripsi detail kegiatan disini..." required>{{ old('content', $post->content) }}</textarea>
                        @error('content')
                            <p class="text-xs text-red-500 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Thumbnail -->
                    <div class="space-y-3">
                        <label for="thumbnail" class="block text-sm font-bold text-gray-700 dark:text-gray-300">Thumbnail Foto</label>
                        
                        @if(!empty($post->thumbnail))
                            <div class="w-48 aspect-video rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
                        ...
                        <input type="file" name="thumbnail" id="thumbnail" 
                               class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 focus:outline-none @error('thumbnail') border-red-500 @enderror">
                        <p class="text-xs text-gray-400">File berupa gambar (JPG, JPEG, PNG). Maksimal 2MB.</p>
                        @error('thumbnail')
                            <p class="text-xs text-red-500 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex justify-end gap-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                        <a href="{{ route('admin.posts.index') }}" class="px-6 py-3 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-900 border border-gray-250 dark:border-gray-700 text-center transition-colors">
                            Batal
                        </a>
