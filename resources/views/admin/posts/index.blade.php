<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            ...
            <a href="{{ route('admin.posts.create') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition-colors duration-200 inline-block text-center">
                + Tambah Kegiatan
            </a>
        </div>
    </x-slot>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700">
                                <th class="p-6 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Thumbnail</th>
                                <th class="p-6 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Judul Kegiatan</th>
                                <th class="p-6 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Kategori</th>
                                <th class="p-6 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Status</th>
                                <th class="p-6 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">Tanggal Rilis</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700 text-sm">
                            @forelse($posts as $post)
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors duration-150">
                                    <!-- Thumbnail -->
                                    <td class="p-6">
                                        <div class="w-16 h-10 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-900 flex items-center justify-center">
                                    ...
                                    <!-- Title -->
                                    <td class="p-6 font-semibold text-gray-900 dark:text-white max-w-xs truncate">
                                        {{ $post->title }}
                                    </td>
                                    <!-- Category -->
                                    <td class="p-6">
                                        <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-bold bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 border border-indigo-100/50 dark:border-indigo-900/50">
                                            {{ $post->category }}
                                        </span>
                                    </td>
                                    <!-- Status -->
                                    ...
                                    <td class="p-6 text-right">
                                        <div class="flex items-center justify-end gap-3">
                                            <a href="{{ route('admin.posts.edit', $post->id) }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300">
                                                Edit
                                            </a>
                                            
                                            <!-- Delete Form -->
                                            <form action="{{ route('admin.posts.destroy', $post->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kegiatan ini?');">
                                                @csrf
                                                @method('DELETE')
