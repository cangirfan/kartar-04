    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-2xl border border-gray-150 dark:border-gray-700 p-6 sm:p-10">
                ...
                <form action="{{ route('admin.umkms.update', $umkm->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $umkm->whatsapp) }}" 
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="description" id="description" rows="4" 
                                  ...
                                  placeholder="Deskripsikan produk yang dijual atau layanan yang ditawarkan..." required>{{ old('description', $umkm->description) }}</textarea>
                        @error('description')
                            <p class="text-xs text-red-500 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location', $umkm->location) }}" 
                    ...
                    <div class="flex items-center gap-3 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                        <input type="checkbox" name="status" id="status" value="1" {{ old('status', $umkm->status) ? 'checked' : '' }}
